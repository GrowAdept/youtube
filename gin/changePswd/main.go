...
func main() {
	...
	authRouter.GET("/changepswd", ChPswdGEThandler)
	authRouter.POST("/changepswd", ChPswdPOSThandler)
...
func profileHandler(c *gin.Context) {
	...
	if auth, ok := session.Values["authenticated"].(bool); !ok || !auth {
		fmt.Println("not authenticated, auth:", auth, "ok:", ok)
	...
	if !ok {
		fmt.Println("missing userid, u.ID:", u.ID, "ok:", ok)
		c.HTML(http.StatusForbidden, "login.html", nil)
		return
	}
	err := u.getUserByID()
	if err != nil {
		fmt.Println("error retrieving user by ID, err:", err)
		c.HTML(http.StatusForbidden, "login.html", nil)
		return
	}
...
func registerPOSThandler(c *gin.Context) {
	...
	err = u.New()
	if err != nil {
		fmt.Println("func (u *User)New err:", err)
...
func emailRecovHandler(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "emailchangepswd.html", gin.H{
		"user":    u,
		"verPass": linkVerPass,
	})
}
...
func emailChPswdHandler(c *gin.Context) {
	...
	err := u.getUserByUsername()
	if err != nil {
		c.HTML(http.StatusBadRequest, "emailchangepswd.html", gin.H{
			"user":    u,
			"verPass": linkVerPass,
...
func ChPswdGEThandler(c *gin.Context) {
	c.HTML(http.StatusOK, "changepswd.html", nil)
}

// even if user is logged in, we ask for the password again incase user was away
// from keyboard but still logged in
func ChPswdPOSThandler(c *gin.Context) {
	var u User
	session, _ := store.Get(c.Request, "session")
	// session struct has field Values map[interface{}]interface{}
	...
	if err != nil {
		fmt.Println("error retrieving user by id, err:", err)
		c.HTML(http.StatusUnauthorized, "changepswd.html", gin.H{"message": "there was an issue changing password"})
		return
	}
	// make sure password was re-entered the same
	newPswd1 := c.PostForm("newpswd1")
	newPswd2 := c.PostForm("newpswd2")
	if newPswd1 != newPswd2 {
		c.HTML(http.StatusBadRequest, "changepswd.html", gin.H{
			"message": "new password and re-enter fields do not match",
		})
	}
	// validate password
	u.password = newPswd1
	...
	formPswd := c.PostForm("oldpswd")
	err = bcrypt.CompareHashAndPassword([]byte(u.pswdHash), []byte(formPswd))
	fmt.Println("bcrypt.CompareHashAndPassword err:", err)
	if err == nil {
		fmt.Println("copareHashAndPassword, err == nil")
		var hash []byte
		hash, err = bcrypt.GenerateFromPassword([]byte(newPswd1), bcrypt.DefaultCost)
		if err != nil {
			fmt.Println("bcrypt error generating password hash, err:", err)
			c.HTML(http.StatusInternalServerError, "changepswd.html", gin.H{
				"message": "there was an issue changing password",
			})
			return
		}
		u.pswdHash = string(hash)
		...
		c.HTML(http.StatusOK, "login.html", gin.H{
			"message": "password sucessfully updated, please login",
		})
		return
	}
	c.HTML(http.StatusUnauthorized, "changepswd.html", gin.H{"message": "check password is entered correctly"})
}
